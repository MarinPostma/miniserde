...
impl<E: VisitorError, T: Deserialize<E>> Deserialize<E> for Box<T> {
    fn begin(out: &mut Cell<Self, E>) -> &mut dyn Visitor<Error = E> {
        ...
        impl<'a, E: VisitorError + 'static, T: Deserialize<E>> Seq<E> for BoxSeq<'a, T, E> {
        ...
        impl<'a, E: VisitorError + 'static, T: Deserialize<E>> Map<E> for BoxMap<'a, T, E> {
    ...
    fn begin(out: &mut Cell<Self, E>) -> &mut dyn Visitor<Error = E> {
        impl<E: VisitorError, T: Deserialize<E>> Visitor for Place<Option<T>, E> {
            ...
            fn boolean(&mut self, b: bool) {
                ...
                Deserialize::begin(&mut cell).boolean(b);
                self.out = cell.map(Option::Some);
            }

            fn string(&mut self, s: &str) {
                let mut cell = Cell::Empty;
                Deserialize::begin(&mut cell).string(s);
                self.out = cell.map(Option::Some);
            }

            fn negative(&mut self, n: i64) {
                let mut cell = Cell::Empty;
                Deserialize::begin(&mut cell).negative(n);
                self.out = cell.map(Option::Some);
            }

            fn nonnegative(&mut self, n: u64) {
                let mut cell = Cell::Empty;
                Deserialize::begin(&mut cell).nonnegative(n);
                self.out = cell.map(Option::Some);
            }

            fn float(&mut self, n: f64) {
                let mut cell = Cell::Empty;
                Deserialize::begin(&mut cell).float(n);
                self.out = cell.map(Option::Some);
            }
            ...
            fn seq(&mut self) -> Option<Box<dyn Seq<E> + '_>> {
                self.out.set(None);
                // Place<Cell<Self, E>>
                self.inner.as_mut() -> Option<&mut Option<T>>
                &mut Option<T>
                &mut Cell<T>
                &mut Cell<T>
                Deserialize::begin(self.out.map(|v| v.as_mut().unwrap())).seq()
            }
